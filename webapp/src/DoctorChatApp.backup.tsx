...
import { Add24Regular, Bot24Regular, History24Regular, Person24Regular, Send24Regular } from '@fluentui/react-icons';
...
// 定义多聊天会话数据结构
interface DoctorChatHistoryData {
...
// 保持向后兼容的旧格式
interface LegacyChatHistoryData {
    doctorInfo: DoctorInfo;
    messages: Message[];
    chatSession: IChatSession | null;
    timestamp: number;
...
const DoctorChatApp: React.FC = () => {
    ...
    const [_isHistoryDrawerOpen, setIsHistoryDrawerOpen] = useState(false);
    ...
    const saveChatSessions = useCallback((doctorInfo: DoctorInfo) => {
        if (!doctorInfo || !currentChatId) return;
    ...
    }, [doctorInfo, currentChatId, chatSessions, messages, chatSession, generateChatTitle]);
    ...
    // 保存聊天历史到本地存储 - 已改为saveChatSessions函数处理
    ...
    const loadChatHistory = (doctorId: string) => {
        try {
            ...
            if (savedData) {
                const chatData = JSON.parse(savedData) as LegacyChatHistoryData;
    ...
    const createNewChat = useCallback(() => {
        ...
        // 如果当前聊天有内容，先保存到历史记录
        if (messages.length > 1) { // 排除只有欢迎消息的情况
            saveChatSessions(doctorInfo);
        }
